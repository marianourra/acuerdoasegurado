...
export type Company = {
  id: string;
  name: string;
  /** Logo usado en reclamos y resto del sitio. */
  logo_url: string | null;
  /** Logo usado en el carrete de la pantalla de login. */
...
export async function getCompanies(): Promise<{ data: Company[] | null; error: { message: string } | null }> {
  const { data, error } = await supabase
    .from('companies')
    .select('id, name, logo_url, login_logo_url')
...
export async function updateCompany(
  id: string,
  params: { name?: string; logo_url?: string | null; login_logo_url?: string | null }
): Promise<{ data: Company | null; error: { message: string } | null }> {
  const patch: { name?: string; logo_url?: string | null; login_logo_url?: string | null } = {};
  ...
  if (params.login_logo_url !== undefined) {
    patch.login_logo_url = params.login_logo_url?.trim() || null;
  }
